...
class metrics_span(nn.Module):
    ...
    def forward(self, logits, labels):
        '''
        logits : {batch_size,max_seq_len,out_features,num_label}
        labels : # {batch_size,max_seq_len,max_seq_len}
        '''
        logits = torch.argmax(logits,dim=-1) # {batch_size,max_seq_len,out_features}
